...
<body>
            ...
            <div class="row">
                ...
                <div class="col-lg-12">
                    <div class="courses_search_container">
                        <form action="ver_curso_faculdade.php" method="POST" id="courses_search_form" class="courses_search_form d-flex flex-row align-items-center justify-content-start">
                            <!--<input  type="search"  class="courses_search_input" name="pesquisar" placeholder="Procurar Cursos" required="required"> !-->

                            <select name="id_instituicao" style="width: 100%" id="id_universidade" class=" chosen-select">

                                <option>Pesquise por Universidade Privda </option>

                                <?php
                                $result_fau = "SELECT * FROM instituicao_sup where sector='Privada' AND id_tipo=1  ORDER BY id_instituicao ";
                                $resultado_fau = mysqli_query($conecta, $result_fau);
                                while ($row_cat = mysqli_fetch_assoc($resultado_fau)) {
                                    echo '<option value="' . $row_cat['id_instituicao'] . '">' . $row_cat['nome'] . ' | Email: ' . $row_cat['email'] . ' </option>';
                                }
                                ?>
                ...
                <div>
                    ...
                    <div class="col-lg-12" style="margin-left: 30%; text-align: center">
                        <div class="sidebar">
    ...
    <script type="text/javascript">
        $(function() {
            $('#id_universidade').change(function() {
                if ($(this).val()) {
                    $('#id_faculdade').hide();
                    $('.carregando').show();
                    $.getJSON('relacionamento/comuna.ajax.php?search=', {
                        id_universidade: $(this).val(),
                        ajax: 'true'
                    }, function(j) {
                        var options = '<option value="">Selecione uma faculdade</option>';
                        for (var i = 0; i < j.length; i++) {
                            options += '<option value="' + j[i].id_faculdade + '">' + j[i].nome + '</option>';
                        }
                        $('#id_faculdade').html(options).show();
                        $('.carregando').hide();
                    });
                } else {
                    $('#id_faculdade').html('<option value="">– Escolha um Distrito –</option>');
                }
            });
        });
    </script>
        ...
        $(function() {
            $('#id_faculdade').change(function() {
                if ($(this).val()) {
                    $('#id_curso').hide();
                    $('.carregando').show();
                    $.getJSON('relacionamento/curso.ajax.php?search=', {
                        id_faculdade: $(this).val(),
                        ajax: 'true'
                    }, function(j) {
                    ...
                    $('#id_curso').html('<option value="">– Escolha um Distrito –</option>');
                }
            });
        });
    </script>
